import '../model/option.dart';
import '../model/question.dart';

final questionsLangPart = [
  Question(
    text: 'Drums out of the darkness, listen well.',
    options: [
      const Option(text: 'SATANA', isCorrect: false),
      const Option(text: 'SATANA2', isCorrect: true),
      const Option(text: 'SATANA3', isCorrect: false),
      const Option(text: 'SATANA4', isCorrect: false),
    ],
  ),
  // Question(
  //   text: 'Drums beating like thunder straight from Hell.',
  //   options: [
  //     const Option(code: 'A', text: 'SATANA', isCorrect: false),
  //     const Option(code: 'B', text: 'SATANA2', isCorrect: false),
  //     const Option(code: 'C', text: 'SATANAh', isCorrect: false),
  //     const Option(code: 'D', text: 'SATANAsanja', isCorrect: true),
  //   ],
  // ),
  // Question(
  //   text: 'Putin zdoch',
  //   options: [
  //     const Option(code: 'A', text: 'Y', isCorrect: false),
  //     const Option(code: 'B', text: 'E', isCorrect: false),
  //     const Option(code: 'C', text: 'S', isCorrect: false),
  //     const Option(code: 'D', text: 'S', isCorrect: true),
  //   ],
  // ),
  // Question(
  //   text: 'kekw',
  //   options: [
  //     const Option(code: 'A', text: '4.48 Psycsdg24ghosis', isCorrect: true),
  //     const Option(code: 'B', text: 'Hamiltddson', isCorrect: false),
  //     const Option(
  //         code: 'C', text: "Much Ado Abodsfsdgut Nothing", isCorrect: false),
  //     const Option(
  //         code: 'D', text: "The Birthday1121212112 Party", isCorrect: false),
  //   ],
  // ),
  // Question(
  //   text: 'Satan is here to claim His ground!',
  //   options: [
  //     const Option(code: 'A', text: '22323', isCorrect: false),
  //     const Option(code: 'B', text: 'suka', isCorrect: false),
  //     const Option(code: 'C', text: 'bljat', isCorrect: true),
  //     const Option(code: 'D', text: 'pizdez', isCorrect: false),
  //   ],
  // ),
  // Question(
  //   text: ' Theres an earth thats green, theres an earth thats free,',
  //   options: [
  //     const Option(code: 'A', text: 'ohohoh', isCorrect: false),
  //     const Option(code: 'B', text: 'yes', isCorrect: false),
  //     const Option(code: 'C', text: 'no', isCorrect: true),
  //     const Option(
  //       code: 'D',
  //       text: 'brrhhh',
  //       isCorrect: false,
  //     ),
  //   ],
  // ),
  // Question(
  //   text: 'Theres a place for you and a place for me',
  //   options: [
  //     const Option(code: 'A', text: 'n-word', isCorrect: false),
  //     const Option(code: 'B', text: 'fuck', isCorrect: false),
  //     const Option(code: 'C', text: 'miggas', isCorrect: false),
  //     const Option(code: 'D', text: 'g', isCorrect: true),
  //   ],
  // ),
];
